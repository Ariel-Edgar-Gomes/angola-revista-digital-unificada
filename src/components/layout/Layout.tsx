...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="fixed inset-0 bg-white/90 backdrop-blur-[120px] pointer-events-none -z-10" />
      <Header />
      <main className="flex-grow relative">
        <div className="absolute top-0 left-0 w-full h-60 bg-gradient-to-b from-metodista-blue/5 to-transparent -z-5"></div>
        <div className="absolute bottom-0 left-0 w-full h-20 bg-gradient-to-t from-metodista-blue/3 to-transparent -z-5"></div>
        {children}
      </main>
      <Footer />
    </div>
